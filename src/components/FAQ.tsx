    ...
    <section id="faq" data-scroll-section className="section-black relative isolate overflow-hidden px-6 py-20 md:py-24">
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl gap-12 lg:grid-cols-[0.72fr_1.28fr]">
        <div data-scroll-reveal data-axis="x" data-direction="-1">
          <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl border border-cyan-300/20 bg-cyan-300/10 text-cyan-200"><MessageCircleQuestion className="h-6 w-6" /></div>
          <span className="text-xs font-bold uppercase tracking-[0.28em] text-cyan-300">FAQ</span>
          <h2 className="mt-4 text-4xl font-black tracking-tight text-white">Preguntas claras. Respuestas honestas.</h2>
          <p className="mt-5 max-w-sm leading-relaxed text-slate-400">Todo lo importante sobre Kanny, su privacidad y cómo puede acompañarte.</p>
        </div>
          ...
          {FAQS.map((item, index) => {
            ...
            return (
              <div key={item.id} data-gsap-faq-item>
                <div className={`interactive-card overflow-hidden rounded-2xl border backdrop-blur-xl ${isOpen ? 'border-cyan-300/30 bg-cyan-300/[0.07]' : 'border-white/[0.08] bg-slate-900/65'}`}>
                  <button id={triggerId} type="button" onClick={() => setOpen(isOpen ? null : index)} className="flex min-h-14 w-full items-center justify-between gap-5 px-6 py-5 text-left text-sm font-bold text-white outline-none" aria-expanded={isOpen} aria-controls={panelId}>
                    {item.question}<ChevronDown className={`h-4 w-4 shrink-0 text-cyan-300 transition-transform duration-500 ${isOpen ? 'rotate-180' : ''}`} />
                  </button>
                  <AnimatePresence initial={false}>
                    {isOpen && <motion.div id={panelId} role="region" aria-labelledby={triggerId} initial={{ height: 0, opacity: 0, filter: 'blur(8px)' }} animate={{ height: 'auto', opacity: 1, filter: 'blur(0px)' }} exit={{ height: 0, opacity: 0, filter: 'blur(6px)' }} transition={{ duration: 0.5, ease: MODERN_EASE }}><p className="px-6 pb-6 text-sm leading-relaxed text-slate-400">{item.answer}</p></motion.div>}
                  </AnimatePresence>
                </div>
              </div>
            )
